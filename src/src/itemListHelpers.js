...
function toListItem(record){
    ...
    if(getRecordType(record) === "ListItem"){
        return record
    }

    let listItem = thing("ListItem")
    ph.value.set(listItem, "item", record)
    return listItem
...
function toList(records){
    ...
    // Handle already list
    if(h.type.get(records) === "ItemList"){
        return cleanListObject(records)
    }

    // Handle array
    let listObject = thing("ItemList")
    records = Array.isArray(records) ? records : [records]
    for(let record of records){
        listObject = append(listObject, record)
    }
    return listObject
...
function filterListItems(listObject, filterParams, negativeFilterParams, strict = false){
    /**
     ...
     */
    let values = valueHelpers.value.get(listObject, "itemListElement", [])
    let filteredValues = filter(values, filterParams, negativeFilterParams, strict)
...
function cleanListObject(listObject){
    ...
    let firstValue = values.filter(x => getValue(x, "previousElement") === undefined || getValue(x, "position") === 0)?.[0]
    ...
    while(currentValue !== undefined){
        ...
        nextValue = values.filter(x => sameAs(nextValueRef, x) || getValue(x, "position") === count + 1)?.[0]
